	...
	void migrate(double* node, double* dA, int* rm,long* parami, double* paramv, int id) {
		...
		while( d >= DIMSUM_MIN_D ) {
			...
			if( d < min(sb_dd,hb_dd) ) {	// JMB COMMENT -- 10.20.09 -- DISPERSAL OCCURS BEFORE PIXEL BOUNDARY IS CROSSED,
				...
				if(  nextRand(parami,id) <= hb_f(toX(lon1,minlon,maxlon,hb_xsize)+hb_dx,toX(lat1,minlat,maxlat,hb_ysize)+hb_dy) ) {	// JMB COMMENT -- FINDS HARD BORDER VALUE FOR NEXT PIXEL WITH RESPECT TO LONGITUDE AND CHECKS TO SEE IF INDIVIDUAL SURVIVES HARD BORDER CROSSING.
