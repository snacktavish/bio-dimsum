...
#define __DIMSUM_PARAMI_RANDINDEX 8
...
#define __DIMSUM_PARAMI_SB_XSIZE 4
#define __DIMSUM_PARAMI_SB_YSIZE 5
#define __DIMSUM_PARAMI_HB_XSIZE 6
#define __DIMSUM_PARAMI_HB_YSIZE 7
...
#else
#define __DIMSUM_MIN_D 0.001
#define __DIMSUM_KA2_EPSILON 0.00001745329252
#define __DIMSUM_EPSILON 0.0017453292520000
...
extern "C" {
	...
	*/
	...
	void migrate(double* node, double* dA, int* rm,long* parami, double* paramv, int id) {
		...
		int sb_xsize = (int)parami[__DIMSUM_PARAMI_SB_XSIZE];
		int sb_ysize = (int)parami[__DIMSUM_PARAMI_SB_YSIZE];
		int hb_xsize = (int)parami[__DIMSUM_PARAMI_HB_XSIZE];
		int hb_ysize = (int)parami[__DIMSUM_PARAMI_HB_YSIZE];
		...
		while( d >= __DIMSUM_MIN_D ) {
			...
			if( d < min(sb_dd,hb_dd) ) {	// JMB COMMENT -- 10.20.09 -- DISPERSAL OCCURS BEFORE PIXEL BOUNDARY IS CROSSED,
			   ...
			   // node[id*2+__DIMSUM_NODE_lat] = toDeg(lld_latf);//getLatFDDeg(lld);
				...
				if(  nextRand(parami,id) <= hb_f(hb,toX(lon1,minlon,maxlon,hb_xsize)+hb_dx,toX(lat1,minlat,maxlat,hb_ysize)+hb_dy) ) {	// JMB COMMENT -- FINDS HARD BORDER VALUE FOR NEXT PIXEL WITH RESPECT TO LONGITUDE AND CHECKS TO SEE IF INDIVIDUAL SURVIVES HARD BORDER CROSSING.
				...
				if( nextRand(parami,id) <= sb_f(sb,toX(lon1,minlon,maxlon,sb_xsize)+sb_dx, toX(lat1,minlat,maxlat,sb_ysize)+sb_dy) ) {
				...
				if( nextRand(parami,id) <= hb_f(hb,toX(lon1,minlon,maxlon,hb_xsize)+hb_dx,toX(lat1,minlat,maxlat,hb_ysize)+hb_dy) ) {
					rm[id] =  1; //continue childrenloop; // this exits the travel loop immediately, so the current child never gets added to the next generation
	...
	__global__
	void migrateGPU(double* children, int* rm, double* d, double* paramd, long* parami, int offset)
	{
	...
	__host__
	void migrateCPU(double* children, int* rm, double* d, double* paramd, long* parami, int offset)
	{
		#pragma omp parallel for
		for(int id =0;id < parami[__DIMSUM_PARAMI_NUMCHILDREN];id++) {
